...
async def test_criar_usuario(client: AsyncClient):
    response = await client.post("/auth/", json=usuario_cliente)
    assert response.status_code == 201
    data = response.json()
    assert data["nome"] == usuario_cliente["nome"]
...
async def test_criar_usuario_email_duplicado(client: AsyncClient):
    ...
    await client.post("/auth/", json=usuario_cliente)
...
# Teste para listar usuários: permitido apenas aos admins.
@pytest.mark.asyncio
...
async def test_obter_usuario_por_id(client: AsyncClient):
    ...
    response_criar = await client.post("/auth/", json=usuario_admin)
